...
class ElekResUpdate extends Command
{
    use AirtableTrait;

    const TABLE = 'Stat data - E RES';
    /**
     * The name and signature of the console command.
     *
     * @var string
     */
    protected $signature = 'elek-res:update';
    ...
    {
        $this->info("---Electricity Residential Restore started---");
        $this->_update();
        $this->info("---Electricity Residential Restore ended---");
    }
    private function _update()
    {
        ...
        $current_data = [];
        $new_data = [];
        $data = StaticElecticResidential::where('backup_date', $restoreDate)->get()->toArray();
        ...
        while (Session::get('offset') != 'stop') {
            ...
            $json = $this->get('Stat data - E RES', $query);
    ...
    private function _updateRecords($records)
    {
        ...
        $this->patch('Stat data - E RES', $requestData, [
    ...
    function _deleteRecord($data)
    {
        ...
        if (count($requestData['records'])) {
            ...
            $this->delete('Stat data - E RES', $requestData, [
                'error' => function ($ex, $data) {
                    // die($ex->getMessage());
                }
            ]);
