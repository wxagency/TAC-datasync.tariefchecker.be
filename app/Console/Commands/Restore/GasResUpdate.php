...
class GasResUpdate extends Command
{
    use AirtableTrait;
    /**
     ...
     * @var string
     */
    protected $signature = 'gas-res:update';
    ...
    {
        $this->info("---Gas Residential Restore started---");
        $this->_update();
        $this->info("---Gas Residential Restore ended---");
    }
    ...
    private function _update()
    {
        ...
        $current_data = [];
        $new_data = [];
        $data = StaticGasResidential::where('backupdate', $restoreDate)->get()->toArray();
        ...
        while (Session::get('offset') != 'stop') {
            ...
            $json = $this->get('Stat data - G RES', $query);
            ...
            foreach ($json['records'] as $gas) {
                ...
                $current_data[$gas['fields']['PROD ID']] = $gas['id'];
            }
        }
    ...
    private function _updateRecords($records)
    {
        ...
        $this->patch('Stat data - G RES', $requestData, [
    ...
    private function _createRecords($records)
    {
        ...
        $this->post('Stat data - G RES', $requestData, [
            'error' => function ($ex, $data) {
                // die($ex->getMessage());
            }
        ]);
    ...
    function _deleteRecord($data)
    {
        ...
        if (count($requestData['records'])) {
            ...
            $this->delete('Stat data - G RES', $requestData, [
                'error' => function ($ex, $data) {
                    // die($ex->getMessage());
                }
            ]);
