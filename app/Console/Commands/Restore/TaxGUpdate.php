...
class TaxGUpdate extends Command
{
    use AirtableTrait;
     ...
     * @var string
     */
    protected $signature = 'tax-G:update';
    ...
    {
        $this->info("---Tax gas Restore started---");
        $this->_restore();
        $this->info("---Tax gas Restore ended---");
    }
    ...
    private function _restore()
    {
        ...
        $currentData = [];
        $newData = [];
        $data = TaxGas::where('backupdate', $restoreDate)->get()->toArray();
    ...
    function _formatRecord($record, $_id = '')
    {
        ...
        $newData = [
            ...
            'Contribution protected customers'  =>  $this->formatPrice($record['contribution_protected_customers']),
            ...
            'fixed tax'  =>  $this->formatPrice($record['fixed_tax']),
        ];
        $result = [
            'fields' =>  $newData,
        ];
