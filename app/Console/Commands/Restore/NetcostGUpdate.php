...
class NetcostGUpdate extends Command
{
    use AirtableTrait;

    const TABLE = 'Netcosts - G';
    /**
     * The name and signature of the console command.
     *
     * @var string
     */
    protected $signature = 'netcost-G:update';
    ...
    {
        $this->info("---Netcost Gas Restore started---");
        $this->_restore();
        $this->info("---Netcost Gas Restore ended---");
    }
    ...
    private function _restore()
    {
        ...
        $currentData = [];
        $newData = [];
        $data = Netcostgs::where('backupdate', $restoreDate)->get()->toArray();
    ...
    function _formatRecord($record, $_id = '')
    {
        ...
        $newData = [
            ...
            'Fixed term'  =>  $this->formatPrice($record['fixed_term']),
            'Variable term'  =>  $this->formatPrice($record['variable_term']),
            'Reading meter yearly'  =>  $this->formatPrice($record['reading_meter_yearly']),
            'Reading meter monthly'  =>  $this->formatPrice($record['reading_meter_monthly']),
