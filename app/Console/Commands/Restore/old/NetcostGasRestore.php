...
class NetcostGasRestore extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'netcost-G:restore {table_name}{backupdate}';
    ...
    private function _delete()
    {
       ...
       while(Session::get('offset')!='stop') {
            ...
            $query['records'][0]=$value['id'];
                    $request =$client->delete('https://api.airtable.com/v0/applSCRl4UvL2haqK/Netcosts%20-%20G', [
                            'headers' => [
                            'Accept' => 'application/json',
                            'Content-type' => 'application/json',
                            'Authorization' => 'Bearer keySZo45QUBRPLwjL'
    ...
    private function _restorenetcostG()
    {
        $data=Netcostgs::where('backupdate', $this->argument('backupdate'))->get();
        foreach ($data as $value) {
            ...
            $query['records'][0]['fields']['Fixed term']=$value->fixed_term;
            $query['records'][0]['fields']['Variable term']=$value->variable_term;
            $query['records'][0]['fields']['Reading meter yearly']=$value->reading_meter_yearly;
            $query['records'][0]['fields']['Reading meter monthly']=$value->reading_meter_monthly;
            $query['records'][0]['fields']['Transport']=$value->transport;

             try {
                $client = new \GuzzleHttp\Client();
               
                $request = $client->post('https://api.airtable.com/v0/applSCRl4UvL2haqK/Netcosts%20-%20G', 
                [
                    'headers' => [
                        'Accept' => 'application/json',
