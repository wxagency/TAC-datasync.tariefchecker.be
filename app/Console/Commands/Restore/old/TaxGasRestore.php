...
class TaxGasRestore extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'tax-gas:restore {table_name}{backupdate}';
    ...
    private function _delete()
    {
       ...
       while(Session::get('offset')!='stop') {
            ...
            $query['records'][0]=$value['id'];
                    $request =$client->delete('https://api.airtable.com/v0/applSCRl4UvL2haqK/Tax%20-%20G', [
                            'headers' => [
                            'Accept' => 'application/json',
                            'Content-type' => 'application/json',
                            'Authorization' => 'Bearer keySZo45QUBRPLwjL'
    ...
    private function _gasRestore()
    {
        $data=TaxGas::where('backupdate', $this->argument('backupdate'))->get();
        foreach ($data as $value) {
            ...
            $query['records'][0]['fields']['Contribution protected customers']=$value->contribution_protected_customers;
            ...
            $query['records'][0]['fields']['fixed tax']=$value->fixed_tax;

             try {
                $client = new \GuzzleHttp\Client();
               
                $request = $client->post('https://api.airtable.com/v0/applSCRl4UvL2haqK/Tax%20-%20G', [
                    'headers' => [
                        'Accept' => 'application/json',
