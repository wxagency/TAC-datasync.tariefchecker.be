...
class staticElectricpro extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'staticElectricpro:start {table_name}{backupdate}';
    ...
    public function handle()
    {
        ...
        $data=StaticElectricProfessional::where('backupdate',$backupdate)->get();
     ...
     if($value->fixed_indiable!=""){
      ...
      if($value->green_percentage!=""){
         $query['records'][0]['fields']['Green Percentage']=$value->green_percentage;
     }
         if($value->origin!=""){
         ...
         $query['records'][0]['fields']['VL']=$value->VL;
          ...
          if($value->NA!=""){     
         $query['records'][0]['fields']['WA']=$value->NA;
         }   
         if($value->BR!=""){      
        ...
        // $query['records'][0]['fields']['PACKS PAR']=date("d/m/Y", strtotime($value->date));
         //$query['records'][0]['fields']['Dyn data - E PAR']=date("d/m/Y", strtotime($value->date));
         ...
         try {
                ...
                $request = $client->post('https://api.airtable.com/v0/applSCRl4UvL2haqK/ELEK%20PRO', [
                    'headers' => [
    ...
    private function _delete()
    {
       ...
       while(Session::get('offset')!='stop') {
           try {
               ...
               $request = $client->get('https://api.airtable.com/v0/applSCRl4UvL2haqK/ELEK%20PRO', [
                   'headers' => [
                       'Accept' => 'application/json',
        ...
        if(!empty($json['records'])) {
            ...
            $query['records'][0]=$value['id'];
                    $request =$client->delete('https://api.airtable.com/v0/applSCRl4UvL2haqK/ELEK%20PRO', [
                            'headers' => [
                            'Accept' => 'application/json',
                            'Content-type' => 'application/json',
                            'Authorization' => 'Bearer keySZo45QUBRPLwjL'
