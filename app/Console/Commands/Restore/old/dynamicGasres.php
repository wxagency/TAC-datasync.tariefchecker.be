...
class dynamicGasres extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'dynamicGasres:start {table_name}{backupdate}';
    ...
    public function handle()
    {
        ...
        $data=DynamicGasResidential::where('backupdate',$backupdate)->get();

        foreach ($data as  $value) {
             $supplier=Supplier::where('commercial_name',$value->supplier)->first();
             $pro=StaticGasResidential::where('product_id',$value->product_id)->first();
        ...
        $query['records'][0]['fields']['Date']=date("d/m/Y", strtotime($value->date));
         ...
         try {
                ...
                $request = $client->post('https://api.airtable.com/v0/applSCRl4UvL2haqK/Dyn%20data%20-%20G%20RES', [
                    'headers' => [
           ...
           if (isset($json['offset'])) {
             ...
             if(!empty($json['records'])) {
                ...
                $query['records'][0]=$value['id'];
                        $request =$client->delete('https://api.airtable.com/v0/applSCRl4UvL2haqK/Dyn%20data%20-%20G%20RES', [
                                'headers' => [
                                'Accept' => 'application/json',
                                'Content-type' => 'application/json',
                                'Authorization' => 'Bearer keySZo45QUBRPLwjL'
