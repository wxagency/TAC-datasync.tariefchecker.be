...
class GasProUpdate extends Command
{
    use AirtableTrait;
    /**
     ...
     * @var string
     */
    protected $signature = 'gas-pro:update';
    ...
    {
        $this->info("---Gas Professional Restore started--");
        $this->_update();
        $this->info("---Gas professional Restore ended--");
    }
    ...
    private function _update()
    {
        ...
        $current_data = [];
        $new_data = [];
        $data = StaticGasProfessional::where('backupdate', $restoreDate)->get()->toArray();
        ...
        while (Session::get('offset') != 'stop') {
            ...
            $json = $this->get('Stat data - G PRO', $query);
    ...
    private function _updateRecords($records)
    {
        ...
        $this->patch('Stat data - G PRO', $requestData, [
    ...
    private function _createRecords($records)
    {
        ...
        $this->post('Stat data - G PRO', $requestData, [
            'error' => function ($ex, $data) {
                // die($ex->getMessage());
            }
        ]);
    ...
    function _deleteRecord($data)
    {
        ...
        if (count($requestData['records'])) {
            ...
            $this->delete('Stat data - G PRO', $requestData, [
                'error' => function ($ex, $data) {
                    // die($ex->getMessage());
                }
            ]);
